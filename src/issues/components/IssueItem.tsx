...
export const IssueItem: FC<Prop> = ({issue}) => {
  ...
  return (
    ...
    className="animate-fadeIn flex items-center px-2 py-3 mb-5 border rounded-md bg-slate-900 hover:bg-slate-800">
      {
        issue.state === State.Close ? <FiCheckCircle size={30} color="green" /> : <FiInfo size={30} color="red" className="min-w-10" />
      }
        ...
        <div className='flex flex-wrap'>
          {
            issue.labels.map((label) => (
              <span key={label.id} className='px-2 mx-2 py-1 text-xs text-white rounded-md'
                style={{
